...
export const projectIssuesService = {
  ...
  async getDepartmentActiveIssues(departmentId: string): Promise<(ProjectIssue & { project?: { id: string; name: string } })[]> {
    ...
    // First, get all project IDs for this department
    ...
    if (linkError) {
      console.error('Error fetching department projects:', linkError);
      throw linkError;
    }
    ...
    const projects = projectDepartmentLinks.map((link: any) => link.projects).filter(Boolean);
