...
export type Range = 'daily' | 'monthly' | 'quarterly' | 'yearly';
...
const getsDaysIn = (range: Range) => {
  ...
  if (range === 'monthly' || range === 'quarterly') {
    ...
    fn = (_date: Date) => 365; // Approximate days in a year
  }
...
const getDifferenceIn = (range: Range) => {
  ...
  if (range === 'monthly' || range === 'quarterly') {
    ...
    fn = (date1: string | number | Date, date2: string | number | Date) => Math.floor(differenceInDays(date1, date2) / 365);
  }
...
const getDateByMousePosition = (context: GanttContextProps, mouseX: number) => {
  ...
  if (context.range === 'daily') {
    // For daily view, each column represents one day
  ...
  return gantt.timelineData.map((year:any) => (
    <div className="relative flex flex-col" key={year.year}>
      <GanttContentHeader
        ...
        renderHeaderItem={(item: number) => (
          <p>
            {format(new Date(year.year, item, 1), 'MMM')}
          </p>
        )}
        title={`${year.year}`}
      />
...
export type GanttColumnProps = {
  ...
  isWeekendColumn?: (item: number) => boolean;
};
  ...
  return (
    ...
    <div
      className={cn(
        ...
        isWeekendColumn?.(index) ? 'bg-red-50' : ''
      )}
...
export const GanttProvider: FC<GanttProviderProps> = ({
  ...
  return (
    ...
    >
      <div
        className={cn(
          'gantt relative grid h-full w-full flex-none select-none overflow-auto rounded-sm bg-secondary touch-pan-x touch-pan-y',
          range,
          className
        )}
        ref={scrollRef}
