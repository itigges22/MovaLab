...
export function KanbanConfigDialog({ 
  ...
  const handleSave = async () => {
    ...
    try {
      ...
      const dbAccess = await accountKanbanConfigService.testDatabaseAccess();
      if (!dbAccess) {
        console.error('Database access failed. Please check the console for details.');
        return;
      }
      
      const updatedConfig = await accountKanbanConfigService.updateAccountKanbanConfig(accountId, columns);
      if (updatedConfig) {
        onColumnsUpdated(columns);
        setOpen(false);
        console.log('Kanban configuration updated successfully');
      } else {
        console.error('Failed to update kanban configuration - no data returned');
      }
    } catch (error) {
