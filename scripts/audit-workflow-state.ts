...
// Interfaces for typed database results
interface WorkflowNodeWithTemplate {
  id: string;
...
async function auditWorkflowState() {
  ...
  if (nodesError) {
    ...
    // Group by template
    const nodesByTemplate = ((nodes as unknown as WorkflowNodeWithTemplate[]) || []).reduce((acc: Record<string, WorkflowNodeWithTemplate[]>, node: WorkflowNodeWithTemplate) => {
    ...
    Object.entries(nodesByTemplate).forEach(([templateName, templateNodes]) => {
      console.log(`  📝 ${templateName}: ${templateNodes.length} nodes`);
      templateNodes.forEach((node: WorkflowNodeWithTemplate) => {
  ...
  let enrichedInstances: EnrichedInstance[] = [];
  ...
  if (instancesError) {
    ...
    // Group by status
    const statusCounts: Record<string, number> = (enrichedInstances || []).reduce((acc: Record<string, number>, inst: EnrichedInstance) => {
    ...
    if (enrichedInstances && enrichedInstances.length > 0) {
      ...
      enrichedInstances.slice(0, 10).forEach((inst: EnrichedInstance) => {
  ...
  let enrichedHistory: EnrichedHistory[] = [];
  ...
  if (historyError) {
    ...
    if (enrichedHistory && enrichedHistory.length > 0) {
      enrichedHistory.forEach((h: EnrichedHistory) => {
  ...
  if (assignmentsError) {
    ...
    if (enrichedAssignments && enrichedAssignments.length > 0) {
      enrichedAssignments.forEach((a: EnrichedAssignment) => {
  ...
  if (allHistory && nodes) {
    const nodeIds = new Set((nodes as unknown as WorkflowNodeWithTemplate[]).map((n: WorkflowNodeWithTemplate) => n.id));
    const badHistory = allHistory.filter((h: HistoryRecord) => {
  ...
  if (instances && nodes) {
    const nodeIds = new Set((nodes as unknown as WorkflowNodeWithTemplate[]).map((n: WorkflowNodeWithTemplate) => n.id));
    const badInstances = instances.filter((inst: InstanceRecord) =>
